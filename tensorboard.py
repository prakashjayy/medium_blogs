...
data= np.array([io.imread(filelist[i]) for i in np.arange(len(filelist[0:100]))]) # Four Dimensional Array with [Batch,height,width,channels]

data = data[:,:,:,0:3] #Taking only the first 3 layers and ignoring the 3rd layer
data = data.astype(np.float32)

indexlist = list()
for i in range(len(filelist[0:100])):
    m = filelist[i]
    indexlist.append(m)
# Dependent Variable 
...
with tf.Session(graph=mnist_graph) as sess:
	...
	for i in range(n_iterations):
		...
		if i%1 ==0:
			save_path = saver.save(sess,"/tmp/model.mnist1")
...
#### Model Building is done and we need to test the model on external data -- Test Data 
test_filelist = glob.glob("/Users/Satish/Downloads/MNIST/Train/Images/test/*.png") #create a list of all filenames.
x = np.arange(len(test_filelist)) #49001
...
with mnist_graph.as_default():
	...
	w7 = init_weights([84,10])
	b7 = bias([10])
	test_pyx = tf.nn.softmax(model(test_data,w1,w2,w3,w4,w5,w6,w7,b1,b2,b3,b4,b5,b6,b7),name="test") # No need to keep the probabilities here. We will use the entire network.


with tf.Session(graph=mnist_graph) as sess:
	tf.initialize_all_variables().run()
	result= sess.run((tf.argmax(sess.run(test_pyx),1)))
	d = {"filename":indexlist_new,"label":result}
	df  = pd.DataFrame(data=d)
	df.to_csv("/Users/Satish/Downloads/MNIST/submission2.csv")
